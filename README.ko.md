...
매니페스트에는 아직 `dependencies: { "soksak-plugin-git-core": "^0.1.0" }` 가 남아 있습니다.
이건 이 플러그인의 선택이 **아닙니다**: 코어의 cross-plugin 게이트가 대상 **플러그인 id** 가
`dependencies` 에 있을 때만 호출을 허용하므로, 계약-핀 소비자는 이 줄 없이는 제공자에 닿지
못합니다. **코어가 소비자 축 계약-핀 선언을 받아들이면 이 줄을 지웁니다** — 코드는 이미 구현체
이름을 부르지 않으므로 코드 변경은 없습니다. 검증: git-core 를 끄고 계약의 다른 구현체를 켠 뒤
`files` 를 다시 실행합니다.
